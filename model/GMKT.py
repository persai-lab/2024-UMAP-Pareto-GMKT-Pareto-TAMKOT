...
class GMKT(nn.Module):
    ...
    def __init__(self, config, q_q_neighbors, q_l_neighbors, l_q_neighbors, l_l_neighbors):
        ...
        # self.value_dim = config.key_dim + self.embeding_size_a
        self.value_dim = config.value_dim
        self.summary_dim = config.summary_dim
        # self.key_matrix = torch.nn.Parameter(torch.Tensor(self.num_concepts, self.key_dim)).to(self.device)
        ...
        self.GNN_Q_L = nn.Linear(self.embeding_size_l, self.embeding_size_q, bias=False) # Q's GNN embedding from L: L embed_size to Q
        self.GNN_L_L = nn.Linear(self.embeding_size_l, self.embeding_size_l, bias=False)
        self.GNN_L_Q = nn.Linear(self.embeding_size_q, self.embeding_size_l, bias=False) # L's GNN embedding from Q: Q embed_size to L
    ...
    def forward(self, q_data, a_data, l_data, d_data):
        ...
        for i in range(1, seq_len-1):
            ...
            # q = self.GNN_Q(q + self.lambda_q * q_q_agg)
            q = self.GNN_Q(q + self.lambda_q * q_q_agg + (1- self.lambda_q) * q_l_agg)
            # l = self.GNN_L(l + self.lambda_l * l_l_agg)
            ...
            lnmt_embedded = (1-d_t) * q_read_key + d_t * l_read_key # learning material embedding mapped to concept for getting knowledge
            ...
            # q_next = self.GNN_Q(q_next + self.lambda_q * q_q_agg + (1- self.lambda_q) * q_l_agg)
        ...
        # batch_pred_type = torch.cat(batch_pred_type[:-1], dim=-1)
    ...
    def read(self, correlation_weight, d_t):
        ...
        value_matrix_reshaped = torch.transpose(d_t*torch.transpose(self.T_QQ(self.value_matrix), 0, 1) + (1-d_t) * torch.transpose(self.T_LQ(
    ...
    def write(self, correlation_weight, qa_embed, l_embed, d_t, d_t_1):
        ...
        memory_after_erase = torch.transpose((((1 - d_t) * (1 - d_t_1)) * torch.transpose(self.T_QQ(self.value_matrix), 0, 1)) + (d_t * d_t_1) * torch.transpose(self.T_LL(
            self.value_matrix), 0, 1) + ((1 - d_t_1) * d_t) * torch.transpose(self.T_QL(self.value_matrix), 0, 1) + (d_t_1 * (1 - d_t)) * torch.transpose(self.T_LQ(
